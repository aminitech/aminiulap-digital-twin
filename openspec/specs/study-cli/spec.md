# study-cli Specification

## Purpose

Make the pipeline reusable beyond the Barbados pilot: an interactive CLI wizard
(`ulap-scope init`) that captures a new study's location, projection, signals,
and geospatial layers into a versioned `study.toml`, with sensible defaults and
recommendations for anything the user doesn't have.
